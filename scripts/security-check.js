...
class SecurityChecker {
	...
	checkVersionConsistency() {
		...
		try {
			...
			const manifestJson = JSON.parse(
				fs.readFileSync(path.join(this.rootDir, 'manifest.json'), 'utf8')
			)
			const versionsJson = JSON.parse(
				fs.readFileSync(path.join(this.rootDir, 'versions.json'), 'utf8')
			)
			...
			if (packageVersion !== manifestVersion) {
				this.error(
					`版本不一致: package.json (${packageVersion}) != manifest.json (${manifestVersion})`
				)
				return false
			}
	...
	checkRequiredFiles() {
		...
		const requiredFiles = ['package.json', 'manifest.json', 'main.ts', 'styles.css', 'CHANGELOG.md']
		...
		try {
			...
			if (fs.existsSync(mainTsPath)) {
				...
				for (const dep of dependencies) {
					if (
						!usedImports.includes(dep) &&
						dep !== '@aws-sdk/client-s3' &&
						dep !== '@aws-sdk/s3-request-presigner'
					) {
