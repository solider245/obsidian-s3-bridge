...
export function loadS3Config(plugin: Plugin): S3Config {
	...
	// 尝试从缓存获取
	const cached = configCache.get<S3Config>(cacheKey)
	if (cached) {
		return cached
	}
	...
	// 缓存配置（5分钟TTL）
	configCache.set(cacheKey, compat, 5 * 60 * 1000)
...
export function buildPublicUrl(plugin: Plugin, key: string): string {
	...
	if (p.providerType === 'cloudflare-r2') {
		...
		throw new Error(
			'Cloudflare R2 需要在配置中填写 Public Base URL（例如 https://<bucket>.r2.dev 或你的自定义域）以生成正确的公开链接'
		)
	}
	...
	if (!ep) {
		...
		throw new Error(
			'缺少 Public Base URL，且无法从 endpoint 推导公开访问地址，请在配置中填写 Public Base URL'
		)
	}
...
export function saveS3Config(plugin: Plugin, config: S3Config): void {
	...
	const data = readProfilesFile(plugin)
	const current =
		data.profiles.find(p => p.id === data.currentProfileId) ??
		createEmptyProfile({ name: 'Default' })
	const merged: S3Profile = {
		...current,
