...
function renderLogEntries(
	...
	for (const log of logs) {
		...
		entry.createEl('span', {
			text: log.event.toUpperCase(),
			cls: `ob-s3-log-level ob-s3-log-${log.event}`,
		})
		...
		if (log.event === 'upload_success' && log.details) {
			...
			messageSpan.appendText(
				`File: ${log.details.fileName}, Size: ${sizeMB}MB, Duration: ${duration}, URL: `
			)
...
export function renderActivityLogSection(app: App, containerEl: HTMLElement) {
	...
	const render = () => {
		...
		const updateDisplayedLogs = (filter: string) => {
			displayedLogs = filter === 'all' ? allLogs : allLogs.filter(log => log.event === filter)
			renderLogEntries(app, logContainer, displayedLogs, render)
		}

		new Setting(toolbar).setName(t('Filter by event type')).addDropdown(dropdown => {
		...
		new Setting(toolbar)
			...
			.addButton(button => {
				button.setButtonText(t('Export to Markdown')).onClick(() => {
