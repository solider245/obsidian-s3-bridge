...
export class BatchUploadModal extends Modal {
	...
	private createProgressArea(): void {
		...
		const speedDisplayEl = progressInfo.createDiv({ cls: 'batch-upload-speed-text' })

		// 保存引用
		Object.assign(progressBar, { fillEl: progressFill })
		Object.assign(progressInfo, { textEl: progressTextEl, speedEl: speedDisplayEl })
	}
	...
	private createStatsArea(): void {
		...
		statsGrid
			.createDiv({ cls: 'batch-upload-stat-item' })
			.createEl('div', {
	...
	private updateItemsList(): void {
		...
		items.forEach(item => {
			...
			if (item.status === 'uploading') {
				...
				const itemProgressFill = itemProgressBar.createDiv({
					cls: 'batch-upload-item-progress-fill',
				})
	...
	private updateStats(progress: BatchProgress): void {
		...
		const completedFilesEl = this.statsEl.querySelector(
			'.batch-upload-completed-files'
		) as HTMLElement
	...
	private createEnhancedStatsArea(): void {
		...
		// 当前速度
		statsGrid
			.createDiv({ cls: 'batch-upload-enhanced-stat-item' })
	...
	private createSpeedChartArea(): void {
		...
		const chartContainer = this.speedChartEl.createDiv({
			cls: 'batch-upload-speed-chart-container',
		})
	...
	private updateEnhancedStats(update: EnhancedProgressUpdate): void {
		const currentSpeedEl = this.enhancedStatsEl.querySelector(
			'.batch-upload-current-speed'
		) as HTMLElement
		const averageSpeedEl = this.enhancedStatsEl.querySelector(
			'.batch-upload-average-speed'
		) as HTMLElement
		const peakSpeedEl = this.enhancedStatsEl.querySelector(
			'.batch-upload-peak-speed'
		) as HTMLElement
		const totalTimeEl = this.enhancedStatsEl.querySelector(
			'.batch-upload-total-time'
		) as HTMLElement
	...
	private updateSpeedChart(update: EnhancedProgressUpdate): void {
		const canvas = this.speedChartEl.querySelector(
			'.batch-upload-speed-chart-canvas'
