 ...
 * 文件大小验证器
 */
export function validateFileSize(value: number, min: number = 1, max: number = 100): ValidationResult {
 ...
 * 超时时间验证器
 */
export function validateTimeout(value: number, min: number = 1000, max: number = 300000): ValidationResult {
  if (typeof value !== 'number' || isNaN(value)) {
 ...
 * 正则表达式验证器
 */
export function validateRegex(value: string, pattern: RegExp, errorMessage: string): ValidationResult {
...
export const validationRules = {
  ...
  performance: {
    ...
    multipartThreshold: (value: number) => validateNumberRange(value, 5 * 1024 * 1024, 100 * 1024 * 1024)
  },
