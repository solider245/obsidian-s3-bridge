...
export class EnhancedProgressManager {
   ...
   * 显示通知
   */
  private showNotification(type: 'start' | 'progress' | 'complete' | 'error', update: EnhancedProgressUpdate): void {
    if (!this.notificationOptions.showNotifications) return
    ...
    switch (type) {
      ...
      case 'complete':
        ...
        message = this.notificationOptions.customMessages?.complete || 
          `上传完成: ${update.fileName} (${timeText})`
        duration = 5000
        break
      case 'error':
        message = this.notificationOptions.customMessages?.error || 
...
// 导出便捷函数
export function startEnhancedUpload(id: string, fileName: string, fileSize: number): EnhancedProgressUpdate {
