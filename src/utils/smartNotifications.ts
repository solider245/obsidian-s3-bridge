...
export class SmartNotificationManager {
	 ...
	 * 显示上传开始通知
	 */
	private showUploadStartNotification(
	 ...
	 * 显示上传进度通知
	 */
	private showUploadProgressNotification(
		update: EnhancedProgressUpdate,
		config: NotificationConfig
	): void {
		// 防抖处理，避免频繁通知
	 ...
	 * 显示上传完成通知
	 */
	private showUploadCompleteNotification(
		update: EnhancedProgressUpdate,
		config: NotificationConfig
	): void {
		const message: NotificationMessage = {
	 ...
	 * 显示上传暂停通知
	 */
	private showUploadPausedNotification(
		update: EnhancedProgressUpdate,
		config: NotificationConfig
	): void {
		const message: NotificationMessage = {
	...
	private dismissOldestNotification(): void {
		const oldestNotification = Array.from(this.activeNotifications.values()).sort(
