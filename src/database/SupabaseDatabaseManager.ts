...
export class SupabaseDatabaseManager {
     ...
     * 创建文件记录
     */
    public async createFile(fileData: Omit<DatabaseFile, 'id' | 'created_at' | 'updated_at'>): Promise<DatabaseFile | null> {
     ...
     * 更新文件记录
     */
    public async updateFile(fileId: string, updates: Partial<DatabaseFile>): Promise<DatabaseFile | null> {
        if (!this.client) return null
        ...
        return this.withRetry(async () => {
            ...
            if (options.filter) {
                ...
                if (options.filter.date_range) {
                    query = query.gte('created_at', options.filter.date_range.start)
     ...
     * 创建仓库记录
     */
    public async createVault(vaultData: Omit<DatabaseVault, 'id' | 'created_at' | 'updated_at'>): Promise<DatabaseVault | null> {
        if (!this.client) return null
     ...
     * 创建上传配置
     */
    public async createUploadConfig(configData: Omit<DatabaseUploadConfig, 'id' | 'created_at' | 'updated_at'>): Promise<DatabaseUploadConfig | null> {
        if (!this.client) return null
     ...
     * 订阅仓库变更
     */
    public subscribeToVaultChanges(
        userId: string,
        callback: (payload: any) => void
    ): () => void {
        if (!this.client) return () => {}
