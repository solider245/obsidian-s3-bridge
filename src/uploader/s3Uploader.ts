...
function validateR2ApiEndpoint(ep: string) {
  ...
  if (!/^https?:\/\//i.test(trimmed)) {
    throw new Error('Invalid endpoint: must start with http(s):// and be a Cloudflare R2 API endpoint');
  }
  const u = new URL(trimmed);
  const host = u.hostname.toLowerCase();
  if (!host.endsWith('.r2.cloudflarestorage.com')) {
    if (host.endsWith('.r2.dev')) {
      throw new Error('Invalid endpoint: r2.dev is public access domain, not API endpoint. Use ACCOUNT_ID.r2.cloudflarestorage.com');
    }
    // 其他 S3 兼容端点（如 MinIO）放行，不做强制
  }
  // 若路径中夹带 bucket，拒绝
  if (u.pathname && u.pathname !== '/') {
    throw new Error('Invalid endpoint: do not include bucket in endpoint path, use path-style with separate bucket field');
  }
}
...
function classifyError(e: any): string {
  ...
  if (/ECONNREFUSED|ECONNRESET|EHOSTUNREACH|ENETUNREACH/i.test(code) || /ECONNREFUSED|ECONNRESET/i.test(msg)) {
    return 'Network connection error: connection refused or reset';
  }

  // TLS/证书
  if (/SELF_SIGNED_CERT_IN_CHAIN|UNABLE_TO_VERIFY_LEAF_SIGNATURE|CERT_|SSL_/i.test(code) || /certificate|SSL/i.test(msg)) {
  ...
  // S3 鉴权/权限
  if (/AccessDenied|SignatureDoesNotMatch|InvalidAccessKeyId|ExpiredToken/i.test(msg + ' ' + name + ' ' + code)) {
