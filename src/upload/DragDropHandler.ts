...
export class EditorDragDropHandler implements DragDropHandler {
	...
	private setupEventListeners(): void {
		const editorEl = (this.editor as any).containerEl

		// 全局拖拽事件监听
		this.addEventListener(
			document.documentElement,
			'dragenter',
			this.handleDocumentDragEnter.bind(this)
		)
		this.addEventListener(
			document.documentElement,
			'dragover',
			this.handleDocumentDragOver.bind(this)
		)
		this.addEventListener(
			document.documentElement,
			'dragleave',
			this.handleDocumentDragLeave.bind(this)
		)
		this.addEventListener(document.documentElement, 'drop', this.handleDocumentDrop.bind(this))
	...
	private handleDocumentDrop(event: DragEvent): void {
		...
		// 只在编辑器外处理
		if (!(this.editor as any).containerEl.contains(event.target as Node)) {
