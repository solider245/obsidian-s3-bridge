...
export class MyPluginSettingTab extends PluginSettingTab {
		...
		new Setting(containerEl)
			...
			.addSlider(slider =>
				slider
					.setLimits(1, 10, 1)
					.setValue(
						this.plugin.settings.maxConcurrentUploads ?? DEFAULT_SETTINGS.maxConcurrentUploads!
					)
					.setDynamicTooltip()
					.onChange(async value => {
	...
	private renderProgressSettings(): void {
		...
		new Setting(containerEl)
			...
			.addToggle(toggle =>
				toggle
					.setValue(
						this.plugin.settings.showUploadNotifications ??
							DEFAULT_SETTINGS.showUploadNotifications!
					)
					.onChange(async value => {
		...
		new Setting(containerEl)
			...
			.addSlider(slider =>
				slider
					.setLimits(1, 60, 1)
					.setValue(
						this.plugin.settings.notificationThreshold ?? DEFAULT_SETTINGS.notificationThreshold!
					)
					.setDynamicTooltip()
					.onChange(async value => {
		...
		new Setting(containerEl)
			...
			.addToggle(toggle =>
				toggle
					.setValue(
						this.plugin.settings.showCompletionAlerts ?? DEFAULT_SETTINGS.showCompletionAlerts!
					)
					.onChange(async value => {
	...
	private renderInterfaceSettings(): void {
		...
		new Setting(containerEl)
			...
			.addToggle(toggle =>
				toggle
					.setValue(
						this.plugin.settings.enableEnhancedStats ?? DEFAULT_SETTINGS.enableEnhancedStats!
					)
					.onChange(async value => {
	 ...
	 * 渲染Supabase云同步设置
	 */
	private renderSupabaseSettings(): void {
		const { containerEl } = this

		containerEl.createEl('h3', { text: 'Supabase云同步设置' })

		// 启用Supabase同步
		new Setting(containerEl)
			.setName('启用Supabase同步')
			.setDesc('启用与Supabase数据库的同步功能')
			.addToggle(toggle =>
				toggle
					.setValue(this.plugin.settings.enableSupabaseSync ?? DEFAULT_SETTINGS.enableSupabaseSync!)
					.onChange(async value => {
						this.plugin.settings.enableSupabaseSync = value
						await this.plugin.saveSettings()
					})
			)
		...
		new Setting(containerEl)
			...
			.setDesc('Supabase项目的URL')
			.addText(text =>
				text
					.setPlaceholder('https://your-project.supabase.co')
					.setValue(this.plugin.settings.supabaseUrl ?? DEFAULT_SETTINGS.supabaseUrl!)
					.onChange(async value => {
		...
		new Setting(containerEl)
			...
			.setDesc('Supabase项目的匿名密钥')
			.addText(text =>
				text
					.setPlaceholder('your-anon-key')
					.setValue(this.plugin.settings.supabaseKey ?? DEFAULT_SETTINGS.supabaseKey!)
					.onChange(async value => {
		...
		new Setting(containerEl)
			...
			.addToggle(toggle =>
				toggle
					.setValue(this.plugin.settings.enableAutoSync ?? DEFAULT_SETTINGS.enableAutoSync!)
					.onChange(async value => {
		...
		new Setting(containerEl)
			...
			.addSlider(slider =>
				slider
					.setLimits(5, 120, 5)
					.setValue(this.plugin.settings.syncInterval ?? DEFAULT_SETTINGS.syncInterval!)
					.setDynamicTooltip()
					.onChange(async value => {
